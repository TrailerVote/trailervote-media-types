  ...
  module MediaTypes
    class PushRecipeIosTest < Minitest::Test
      ...
      def test_the_default_media_type
        ...
        assert_equal 'application/vnd.trailervote.push_recipe_ios.v1+json', PushRecipeIos.to_constructable.to_s
      end

      def test_it_registers
        assert_media_types_registered(PushRecipeIos) do
          formatted_mime_type 'application/vnd.trailervote.push_recipe_ios.v%<version>s+json' do
            version 1, symbol: :push_recipe_ios_v1_json, synonyms: []
          end
        end
      end
